...
class MemberajaxHandler {
    ...
    public function register_after_payment_callback() {
        ...
        try {
            if ($plan_type === 'one_time') {
                ...
                $this->handle_one_time_payment($email, $password, $name, $payment_id);
     ...
     */
    private function handle_one_time_payment($email, $password, $name, $payment_id) {
        ...
        if ($user_wp_id) {
            ...
            if ($customer_id) {
                try {
                    ...
                    if ($customer && $customer->id) return $customer->id;
                } catch (\Stripe\Exception\ApiErrorException $e) {
